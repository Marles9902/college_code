...
int main(void){
  ...
  PWM_Output_Init();    // output from PA5, SysTick interrupts
...
//  GPIO_PORTA_DR8R_R |= 0x20;    // Add this line if the PWM is used to drive a DC Motor to enable 8 mA drive on PA5
...
void SysTick_Handler(void){
  ...
  if(GPIO_PORTA_DATA_R&0x20){   // toggle PA5:previous one is a duty cycle
    GPIO_PORTA_DATA_R &= ~0x20; // make PA5 low
