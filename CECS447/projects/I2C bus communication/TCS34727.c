...
uint16_t TCS34727_GET_RAW_GREEN(void){
	...
	/* Concatanate into 16-bit value */
	//CODE_FILL
	GREEN_DATA = (GREEN_DATA << 8) | GREEN_LOW;
...
uint16_t TCS34727_GET_RAW_BLUE(void){
	...
	/* Concatanate into 16-bit value*/
	//CODE_FILL
	BLUE_DATA = (BLUE_DATA << 8) | BLUE_LOW;
...
void TCS34727_GET_RGB(RGB_COLOR_HANDLE_t* RGB_COLOR_Instance){
	...
	*/ 
	...
	//CODE_FILL
	float red_value = TCS34727_GET_RAW_RED();
	float green_value = TCS34727_GET_RAW_GREEN();
	float blue_value = TCS34727_GET_RAW_BLUE();
	
	RGB_COLOR_Instance->R = (float)((red_value/(RGB_COLOR_Instance->R_RAW / RGB_COLOR_Instance->C_RAW)) * 255.0);
	RGB_COLOR_Instance->G = (float)(green_value/(RGB_COLOR_Instance->G_RAW / RGB_COLOR_Instance->C_RAW) * 255.0);
	RGB_COLOR_Instance->B = (float)(blue_value/(RGB_COLOR_Instance->B_RAW / RGB_COLOR_Instance->C_RAW) * 255.0);
