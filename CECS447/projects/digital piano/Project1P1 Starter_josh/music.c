...
extern unsigned int sw_pressed;
...
void play_a_song(void)
{
	...
	while (Score_Tab[current_song][i].delay&&!sw_pressed){
		
		// check music state first before playing
		if (music_state == 0) {
			SysTick_stop();  // stop playing immediately if music is turned off.
			return;
		}
