...
def bisect(l, i_lb, i_ub, target_value, debug=False):
    ...
    Even though the spec we are given idicates we can assume l is already sorted, we will double-check and do the sorting just in case.

    parameters:
        l:      the list (elementas should all be of the same type and comparable)
...
if __name__ == '__main__':
    ...
    fname = "mobysmall.txt"
    run_benchmark_words_file_to_list_series(fname, 1000, debug=False) # set debug to True will produce A LOT of output... use it only if you think something is broken
    print()
